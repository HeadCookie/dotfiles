...
// Bloom samples (same as original)
...
// (Removed: The glitch-inducing ABERRATION_FACTOR and offsetFunction)

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 uv = fragCoord.xy / iResolution.xy;

    // Sample all channels at the same UV to remove the glitchy chromatic offsets.
    vec4 base = texture(iChannel0, uv);
    vec4 source = toOklab(base);
    vec4 dest = source;
    ...
    // Increase saturation in Oklab space (boost the a and b channels)
    float saturationBoost = 1.5;
    dest.yz *= saturationBoost;
